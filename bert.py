"""
1. Reads and preprocesses the training, validation, and test data.
2. Subsamples the data to balance relevant and non-relevant instances.
3. Tokenizes and preprocesses the data using a BERT tokenizer.
4. Trains a BERT model for sequence classification.
5. Evaluates the model on validation data using precision and NDCG metrics.
6. Reranks the test data based on the trained BERT model and saves the top 100 ranked passages for each query.
"""
...
####### Load model
BERTmodel = AutoModelForSequenceClassification.from_pretrained("bert-base-uncased", num_labels=1, use_cache=False)
...
args = TrainingArguments(
    ...
    per_device_train_batch_size=13, # Might have to adjust if memory problems
...
model_trained = AutoModelForSequenceClassification.from_pretrained(outdir + '/bert_model', num_labels=1, use_cache=False)
...
def predict_probabilities(model_trained, data):
    device = torch.device('cuda') if torch.cuda.is_available() else torch.device('cpu') # If GPU is available, use it
...
def mean_ndcg(df, retrieved):
    ...
    for ind, row in tqdm(df[df['relevancy'] == 1].iterrows(), total=df[df['relevancy'] == 1].shape[0]):
        i = row['rank']
        if row['qid'] not in done_qid and counter > 0:
