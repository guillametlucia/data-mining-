"""
This script builds a logistic regression model using Word2Vec embeddings to evaluate the retrieval quality of passages.
It performs the following tasks:
1. Reads and preprocesses the data from Task 1.
2. Trains a Word2Vec model on the cleaned text data.
3. Calculates embeddings for documents and queries.
4. Computes cosine similarity and trains a logistic regression model.
5. Evaluates the model using precision and NDCG metrics.
6. Re-ranks the test data based on logistic regression scores.
7. Saves the results and model data for further analysis.
"""
...
outdir = './outputs/word2vec_logisticregression'
if not os.path.exists(outdir):
    os.mkdir(outdir)

dir_task1 = './outputs/clean_text_and_evaluate_BM25'

############# LOADING THE CLEAN DATA FROM CLEAN_TEXT_AND_EVALUATE_BM25############################
# same sampling as clean_text_and_evaluate_BM25
...
def mini_batch_gradient_descent(X, y, lr, weights, n_iter=10, batch_size=5, verbose=True):
    ...
    for i in tqdm(range(n_iter)):
        ...
        # Investigate results. Look at effect of learning rate and diffferent starting points of theta. See if they converge. 
        if verbose:
