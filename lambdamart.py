"""
Implements the LambdaMART algorithm to rank passages based on BM25 and cosine similarity features. It performs the following tasks:

1. Loads and preprocesses data from previous tasks.
2. Computes BM25 features for training, validation, and test datasets.
3. Trains LambdaMART models with different objectives (NDCG and MAP) and evaluates their performance on validation data.
4. Uses the best model to rerank candidate passages and save the top 100 ranked passages for each query.
...
outdir = './outputs/lambdamart'
if not os.path.exists(outdir):
    os.mkdir(outdir)
dir_task2 = './outputs/word2vec_logisticregression'

############# LOADING THE COSINE SIMILARITY FROM LOGISTIC REGRESSION OUTPUT############################
